...
export const AuthProvider = ({ children }) => {
  ...
  useEffect(() => {
    const storedUser = localStorage.getItem("medira_user");
    if (storedUser) {
  ...
  const login = (userData) => {
    ...
    localStorage.setItem("medira_user", JSON.stringify(userWithToken));
  ...
  const logout = () => {
    setUser(null);
    localStorage.removeItem("medira_user");
    router.push("/");
  };
  ...
  const updateProfile = (newUserData) => {
    try {
      ...
      setUser(updatedUser);
      localStorage.setItem("medira_user", JSON.stringify(updatedUser));
