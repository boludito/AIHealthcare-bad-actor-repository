...
export async function POST(request) {
  try {
    ...
    // 2. Find or create user profile in medira.users
    const mediraDb = db.client.db("medira");
    let mediraUser = await mediraDb
      .collection("users")
      .findOne({ userId: userId });
    ...
    if (mediraUser) {
      console.log("Found user in medira.users collection");

      // Direct update for medira user
      const mediraUpdateResult = await mediraDb
        .collection("users")
        .updateOne({ userId: userId }, { $set: { healthData: healthData } });

      console.log("Medira user update result:", mediraUpdateResult);
    } else {
      console.log("Creating new user in medira.users collection");

      // Create new user in medira.users
      const newMediraUser = {
        userId: userId,
      ...
      const mediraInsertResult = await mediraDb
        .collection("users")
        .insertOne(newMediraUser);
      console.log(
        "Created new user in medira.users:",
        mediraInsertResult
      );
    }
